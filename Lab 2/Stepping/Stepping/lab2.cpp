...
int CustomSearch( const vector<string>& list, string findMe, int& cycleCounter )
{
		...
		while (listEnd-1 != listBeginning)
		{
			cycleCounter++;
			int nextSearch = ((listEnd - listBeginning) / 2) + listBeginning;
			if (findMe == list[nextSearch])
			{
				return nextSearch;
				break;
			}
			else if (findMe > list[nextSearch])
			{
				listBeginning = nextSearch;
			}
			else
			{
				listEnd = nextSearch;
